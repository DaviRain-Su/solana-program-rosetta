...
// Stock Zig's current bpf backend lowers the float->int path in Rent helpers
// via unsupported builtins, so this comparison variant keeps the benchmark on
// the default integer-equivalent thresholds used by ProgramTest.
fn rentExemptionMultiplier(rent: *align(1) const Rent.Data) ?u64 {
    return switch (@as(u64, @bitCast(rent.exemption_threshold))) {
        @as(u64, @bitCast(@as(f64, 1.0))) => 1,
        @as(u64, @bitCast(@as(f64, 2.0))) => 2,
        else => null,
    };
}

fn rentMinimumBalance(rent: *align(1) const Rent.Data, data_len: usize) TokenError!u64 {
    const multiplier = rentExemptionMultiplier(rent) orelse return TokenError.NotRentExempt;
    const total_data_len: u64 = Rent.account_storage_overhead + data_len;
    return total_data_len * rent.lamports_per_byte_year * multiplier;
}

fn rentIsExempt(rent: *align(1) const Rent.Data, balance: u64, data_len: usize) TokenError!bool {
    return balance >= try rentMinimumBalance(rent, data_len);
}
...
fn initializeMint(accounts: []Account, data: []const u8) TokenError!void {
    ...
    if (!(try rentIsExempt(rent, mint_account.lamports().*, mint_account.dataLen()))) {
...
fn initializeAccount(program_id: *align(1) PublicKey, accounts: []Account) TokenError!void {
    ...
    if (!(try rentIsExempt(rent, token_account.lamports().*, token_account.dataLen()))) {
        return TokenError.NotRentExempt;
    }
    ...
    if (mint_account.id().equals(native_mint_id)) {
        const rent_exempt_reserve = try rentMinimumBalance(rent, token_account.dataLen());
...
fn transfer(program_id: *align(1) PublicKey, accounts: []Account, data: []const u8) TokenError!void {
    ...
    var source = try state.Account.fromBytes(source_account.data());
    var destination = try state.Account.fromBytes(destination_account.data());
...
fn mintTo(program_id: *align(1) PublicKey, accounts: []Account, data: []const u8) TokenError!void {
    ...
    var mint = try state.Mint.fromBytes(mint_account.data());
...
fn processInstruction(program_id: *align(1) PublicKey, accounts: []Account, data: []const u8) TokenError!void {
    ...
    switch (instruction_type.*) {
        ...
        ix.InstructionDiscriminant.transfer => {
            ...
            try transfer(program_id, accounts, data);
        },
        ...
        ix.InstructionDiscriminant.mint_to => {
            ...
            try mintTo(program_id, accounts, data);
        },
        ix.InstructionDiscriminant.burn => {
            //sol.log("Instruction: Burn");
            try burn(program_id, accounts, data);
        },
        ix.InstructionDiscriminant.close_account => {
            //sol.log("Instruction: CloseAccount");
            try closeAccount(program_id, accounts);
        },
